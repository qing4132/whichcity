...
> **Version 2.4** — Data governance overhaul complete (2026-04-14)
...
| Dimension | Value |
|-----------|-------|
| Cities | 151 (7 deleted + 7 added in v2.4; +San Jose CA restored) |
| Professions | 25 (includes 数字游民 $85k fixed; 公务员已更名为 政府/NGO行政) |
| Tax Systems | 88 countries + city overrides + 6 expat schemes |
...
│   ├── city/[slug]/           City detail (151 slugs)
...
│   ├── city-detail/           Sub-components (Hero, FeedPost, Nomad, Similar)
│   ├── RankingContent.tsx     ~680 lines
│   ├── CompareContent.tsx     ~800 lines
│   ├── HomeContent.tsx        ~175 lines
│   ├── NavBar.tsx             ~260 lines
│   └── ClimateChart.tsx       ~160 lines
│
├── hooks/useSettings.ts       Global settings
│
├── lib/
│   ├── types.ts               City interface (~60 fields)
│   ├── taxData.ts             88 country tax tables + expat schemes
│   ├── taxUtils.ts            Tax computation engine
│   ├── i18n.ts                ~2300 lines (4 locales, 470+ keys)
│   ├── constants.ts           Regions, flags, currencies
│   ├── dataLoader.ts          Server-side data loading
│   ├── clientUtils.ts         Life Pressure formula, helpers
│   ├── citySlug.ts            ID↔slug mappings
│   ├── cityIntros.ts          151 cities × 4 locales
│   └── cityLanguages.ts       Official languages per city
│
├── data/                      **Data governance layer (v2.4)**
│   ├── README.md              Data architecture overview
│   ├── cities-source.json     Single Source of Truth (151 cities × 39 raw fields)
│   ├── registry.json          Field metadata (49 fields, full lineage)
│   ├── changelog.json         Data version history
│   ├── scripts/               Export pipeline, validation, i18n audit
│   ├── sources/               Raw data source JSONs (ILO, UNODC, GPI, etc.)
│   ├── salary-research/       Salary data collection infrastructure
│   │   ├── raw/               BLS, ILO, Eurostat, Numbeo raw data
│   │   ├── scripts/           Collection + computation scripts
│   │   ├── mappings/          SOC/ISCO/NOC occupation code mappings
│   │   └── salary-estimates-v2.json  Computed salary matrix (3600+ points)
│   └── automation/            Auto-update design plans
│
├── public/data/
│   ├── cities.json            151 cities (auto-generated by export.mjs)
...
## 5. Data Governance (v2.4)

> Full documentation: `data/README.md`

### Single Source of Truth

`data/cities-source.json` is the **only file to edit** for city data. `public/data/cities.json` is auto-generated.

```
data/cities-source.json  (39 raw fields per city)
        │
        ▼  node data/scripts/export.mjs
        │  Computes: homicideRateInv, gpiScoreInv, safety/healthcare/governance indices,
        │            confidence scores (numeric 0-100), averageIncome (professions median)
        ▼
public/data/cities.json  (49 fields per city, never edit manually)
```

### Salary Data System (v2.4)

151 cities × 25 professions = 3,775 salary data points. All values are **pre-tax annual median USD**.

| Tier | Cities | Avg Conf | Source |
|------|--------|----------|--------|
| 1 | 21 US cities | 98 | BLS OEWS May 2024 — direct metro-area median per SOC code |
| 2 | ~90 developed | 50-64 | Numbeo city salary × country-specific profession ratios (80 countries) |
| 3 | ~40 developing | 26-46 | Same method, lower data quality |

**Profession ratio sources by country**:
- **Quality A** (10 countries): JP (厚労省賃金構造基本統計調査), CA (StatCan 14-10-0340), UK (ONS ASHE), DE (Destatis/Entgeltatlas), FR (INSEE DADS), SG (MOM OWS), CH (BFS LSE), KR (KOSIS), CZ (CZSO), NZ (Stats NZ)
- **Quality B** (22 countries): CN (国家统计局), AU (ABS), TW (DGBAS), HK (Census), IL, IN, UA, KZ, RU, Nordics, IE, IT, ES, PT, etc.
- **Quality C** (39 countries): ISCO major group ratios + regional patterns
- **Quality D** (9 countries): Limited/no public occupation wage data (IR, LB, KH, MM, BD, NP, UZ, MN, NG)

**Confidence formula**: `conf = C_base × C_city × C_prof × C_currency × 100`
- C_base: 1.0 (BLS direct) / 0.75 (Numbeo crowdsourced) / 0.4 (no city data)
- C_city: 1.0 (city-specific) / 0.8 (national × city multiplier) / 0.6 (national only)
- C_prof: 0.9 (national occupation data) / 0.65-0.85 (sector/ISCO group) / 0.35 (US proxy)
- C_currency: 1.0 (USD) / 0.95 (stable) / 0.85 (moderate) / 0.7 (volatile)

**数字游民**: Fixed $85,000 globally (Nomads.com 2026 survey). Not location-dependent.

**averageIncome**: Auto-computed as professions median by export.mjs. Not stored in SOT.

### City Changes (v2.4)

**Deleted 6 cities**: 仁川(108), 横滨(139), 尔湾(134), 普拉亚德尔卡门(142), 坎昆(157), 巴亚尔塔港(158)
- Reason: Metro-area duplicates (仁川/横滨/尔湾) or resort towns with poor data (墨西哥3城)

**Added 7 cities**: 维尔纽斯(162), 里加(163), 尼科西亚(164), 圣多明各(165), 基多(166), 阿克拉(167), 亚的斯亚贝巴(168)
- Fills coverage gaps: Baltics (2), Caribbean (1), Ecuador (1), West Africa (1), East Africa (1), Cyprus (1)
- 7 new tax systems added to taxData.ts

**Restored**: 圣何塞(美国)(133) — had been mistakenly deleted; has independent BLS MSA + Numbeo data

### Key Data Files

| File | What | Edit? |
|------|------|-------|
| `data/cities-source.json` | SOT: 151 cities × 39 raw fields | ✓ Edit this |
| `data/registry.json` | Field metadata (49 fields) | Reference |
| `data/salary-research/salary-estimates-v2.json` | Computed salary matrix | Generated |
| `data/salary-research/raw/bls-oews-extracted.json` | BLS OEWS May 2024 (461 records) | Generated |
| `data/salary-research/raw/numbeo-salary-from-cache.json` | Numbeo city salaries (148 cities) | Generated |
| `data/salary-research/raw/country-specific-ratios.json` | 80 country profession ratios | Generated |
| `data/salary-research/mappings/occupation-codes.json` | 25 professions × SOC/ISCO/NOC codes | Reference |

---

## 5b. Responsive Breakpoints
