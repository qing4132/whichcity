...
```
public/data/cities.json          ← 120 个城市的所有数据（薪资、成本、房价、AQI、直飞城市数…）
...
展示单个城市的 7 个关键指标卡片（收入、成本、储蓄、房价、AQI、医生数、直飞城市），卡片边框按百分位着色（绿=前25%，红=后25%，默认=中间）。收入/成本随顶栏的职业和消费层级动态变化。还包含住房&气候信息和相关城市对比链接。
...
120 城市按 5 个维度排序：储蓄率、购买力、购房年限、空气质量、直飞城市数。
...
| `RankingContent.tsx` | 288 | 排行榜表格（5 个 Tab：储蓄/购买力/购房/空气/直飞） |
| `CityDetailContent.tsx` | 254 | 城市详情页（统一顶栏，7 个百分位着色卡片，住房&气候） |
...
| 文件 | 行数 | 作用 |
|------|------|------|
| `i18n.ts` | 1064 | 4 语言翻译（zh/en/ja/es），城市名、国家名、职业名 |
...
| `types.ts` | 43 | TypeScript 类型定义（City, Locale, CostTier…） |
