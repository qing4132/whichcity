...
```
public/data/cities.json          ← 120 个城市的所有数据（薪资、成本、房价、AQI…）
...
CityComparison 是最大的组件（499 行），负责：加载数据、管理所有状态（选中城市、语言、货币、深色模式）、通过 Context 传递给子组件。

### 城市详情 `/city/[slug]`
`app/city/[slug]/page.tsx`（服务端）→ 用 `dataLoader.ts` 读数据 → 传给 `CityDetailContent`（客户端）

展示单个城市所有信息：薪资、成本层级、购房年限、气候、AQI。

### 双城对比 `/compare/[pair]`
`app/compare/[pair]/page.tsx`（服务端）→ 解析 URL 中的两个城市 slug → 传给 `CompareContent`（客户端）

两城指标逐项对比（含职业薪资和消费层级），标出谁赢。顶栏与首页一致，共享职业/消费层级/语言/货币选项。包含 KeyInsights 综合评分。

### 排行榜 `/ranking`
`app/ranking/page.tsx`（服务端）→ 传全部城市给 `RankingContent`（客户端）

120 城市按储蓄率/性价比/房价/空气排序。
...
| `city/[slug]/page.tsx` | 城市详情页，生成 SEO meta + JSON-LD |
...
| 文件 | 行数 | 作用 |
|------|------|------|
| `CityComparison.tsx` | 490 | 首页主组件：城市选择器 + 状态管理 + Context Provider |
| `RankingContent.tsx` | 274 | 排行榜表格 + 排序逻辑 |
| `CityDetailContent.tsx` | 248 | 城市详情页布局 |
| `CompareContent.tsx` | 210 | 双城对比页布局（含顶栏、KeyInsights） |
...
| `PageShell.tsx` | 98 | 子页面公共外壳（导航栏 + 页脚） |
...
| 文件 | 行数 | 作用 |
|------|------|------|
| `i18n.ts` | 1044 | 4 语言翻译（zh/en/ja/es），城市名、国家名、职业名 |
...
| `types.ts` | 42 | TypeScript 类型定义（City, Locale, CostTier…） |
...
| 文件 | 行数 | 作用 |
|------|------|------|
| `useSettings.ts` | 115 | 子页面的设置管理（语言/货币/深色模式/职业/消费层级），读写 localStorage |
...
2. **子页面** — `useSettings` hook 从 localStorage 读取设置（语言、货币、深色模式）
