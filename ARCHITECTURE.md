...
| 文件 | 行数 | 作用 |
|------|------|------|
| `CityComparison.tsx` | 495 | 首页主组件：城市选择器 + 状态管理 + Context Provider |
| `RankingContent.tsx` | 280 | 排行榜表格 + 排序逻辑 |
| `CityDetailContent.tsx` | 210 | 城市详情页布局（统一顶栏，动态职业收入/消费层级） |
| `CompareContent.tsx` | 215 | 双城对比页布局（含顶栏、KeyInsights，胜负从 rows 统计） |
