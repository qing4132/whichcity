...
展示单个城市的 12 个关键指标卡片，分两行每行 6 个显示。第一行：收入💰、消费🛒、储蓄🏦、年工时⏰、时薪💵（=收入÷工时）、巨无霸指数🍔（=城市价÷全球中位数，显示为倍数）；第二行：房价🏠（/m²）、购房年限🔑（=70m²÷年储蓄）、空气质量🌬️、安全🛡️、医疗🩺、直飞✈️。每张卡片副标题统一显示排名（#N / 120）。卡片边框按百分位着色（绿=前25%，红=后25%，默认=中间）。收入/成本随顶栏的职业和消费层级动态变化。还包含气候信息和相关城市对比链接。
...
| `CityDetailContent.tsx` | ~250 | 城市详情页（统一顶栏，12 个百分位着色卡片含排名，气候） |
