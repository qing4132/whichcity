...
```
public/data/cities.json          ← 120 个城市的所有数据（薪资、成本、房价、AQI、直飞城市数、安全指数、年工作时长…）
...
展示单个城市的 12 个关键指标卡片，分两行每行 6 个显示。第一行：收入、消费、储蓄、年工时、时薪（=收入÷工时）、巨无霸指数（=城市价÷全球中位数，显示为倍数）；第二行：房价（/m²）、购房年限（=70m²÷年储蓄）、空气质量、安全、医疗、直飞。每张卡片副标题统一显示排名（#N / 120）。卡片边框按百分位着色（绿=前25%，红=后25%，默认=中间）。收入/成本随顶栏的职业和消费层级动态变化。

气候区块展示 6 个指标：气候类型、年均温、夏冬温差、年均湿度、年降水、日照。

“相似城市”区块基于 11 维指标的欧氏距离（归一化后）在构建时预计算，展示最相似的 6 个城市，每张卡片显示优势维度差异百分比，提供“查看”和“VS”两个链接。

### 双城对比 `/compare/[pair]`
`app/compare/[pair]/page.tsx`（服务端）→ 解析 URL 中的两个城市 slug → 传给 `CompareContent`（客户端）

两城指标逐项对比（含职业薪资、消费层级、工作时长、直飞城市、安全指数），胜负从 rows 数组统计（支持平局）。顶栏与首页一致，共享职业/消费层级/语言/货币选项。包含 KeyInsights 综合评分。

### 排行榜 `/ranking`
`app/ranking/page.tsx`（服务端）→ 传全部城市给 `RankingContent`（客户端）

120 城市按 7 个维度排序：储蓄率、购买力、购房年限、空气质量、直飞城市数、安全指数、年工作时长。所有 Tab 统一使用 emerald（好）/ amber（中）/ red（差）配色。Tab 按钮等宽布局占满整行，表格使用 table-fixed 布局确保切换 Tab 时列宽一致。
...
| `RankingContent.tsx` | ~305 | 排行榜表格（7 个 Tab：储蓄/购买力/购房/空气/直飞/安全/工作时长） |
| `CityDetailContent.tsx` | ~280 | 城市详情页（统一顶栏，12 个百分位着色卡片含排名，气候，相似城市推荐） |
...
| `constants.ts` | 182 | 地区分组、城市旗帜 emoji、气候数据（含夏冬均温、湿度） |
...
| `add-workhours.mjs` | 添加 74 国家年工作时长到 cities.json（国家级数据） |
| `add-climate-detail.mjs` | 添加 120 城市夏冬均温、湿度到 constants.ts |
...
| 年工作时长 | OECD Employment Outlook 2024、ILO ILOSTAT（国家级全行业平均） |
| 气候 | WMO、各国气象局 |
| 夏冬均温、湿度 | WMO Climate Normals 1991–2020、NOAA、各国气象局 |
...
| 30% | 暴力犯罪（反向计分：犯罪率越低得分越高） | Numbeo + UNODC homicide rate |
| 20% | 财产犯罪（反向计分） | Numbeo Property Crime |
| 10% | 外国人友好度 | InterNations（城市级）/ Gallup MAI（国家级回退） |

每个城市附带 `safetyConfidence` 字段（high/medium/low），标记数据可靠度。低置信度城市在详情页和排行榜中以 `*` 标注。
