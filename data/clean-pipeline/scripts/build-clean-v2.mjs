#!/usr/bin/env node
/**
 * build-clean-v2.mjs — Clean pipeline v2.
 *
 * Improvements over v1:
 *   1. costModerate uses WB Price Level Index (PLI = Nominal/PPP GDP)
 *      instead of Gini tiered multiplier. PLI is the standard
 *      national-accounts country price level → ICP compatible.
 *   2. US cities use Zillow ZORI (market median) instead of HUD FMR
 *      (40th-percentile FMR). Zillow ZORI is closer to what users
 *      actually pay, apples-to-apples with ONS/StatCan.
 *   3. cost anchor switches: instead of HFCE/12 × inequality mul, we
 *      use a single US anchor (BEA PCE middle-class core consumption
 *      ex-housing = $3200/mo) and scale by PLI × city premium.
 *
 * Sources (zero Numbeo):
 *   WB PLI (CC BY 4.0) — country-level price index
 *   BEA PCE + BLS CES — US anchor (Public Domain)
 *   Zillow ZORI — 19 US MSAs, market-median rent
 *   ONS Private Rent Feb 2026 — 2 UK cities (OGL v3)
 *   StatCan 34-10-0133-01 — 5 CA CMAs (OGL-Canada)
 *   BEA RPP — US MSA adjustments (Public Domain)
 *   Eurostat PLI — EU country adjustments (CC BY 4.0)
 *
 * Formula:
 *   costModerate = US_ANCHOR × PLI_country × cityPremium × cityPriceAdj
 *     US_ANCHOR = 3200 USD (BEA PCE core urban middle-class, ex-housing)
 *     PLI_country = WB NominalGDPpc / PPPGDPpc
 *     cityPremium ∈ {1.00, 1.10 capital, 1.20 primary, 1.35 city-state like SG/HK}
 *     cityPriceAdj = BEA_RPP/100 (US) OR Eurostat_PLI/100 (EU) OR 1.0
 *
 *   monthlyRent (Tier A gov-direct preferred):
 *     Zillow ZORI (US 19) → market median
 *     ONS (UK 2) → official private rent
 *     StatCan (CA 5) → CMHC 1BR
 *     fallback Tier B = cost × rentShare(GDP-PPP tier)
 *
 *   housePrice = GNI × PTI / 60m²  (same as v1, unchanged)
 */
...
const US_ANCHOR = 3200; // USD/mo, BEA PCE urban middle-class ex-housing core consumption
...
const IMF_OVERRIDES = {
  台湾: { priceLevelIndex: 0.66, gdpPPPPerCapita: 73344, gniPerCapita: 34000 },
};
...
const CITY_STATE_PREMIUM = new Set(["新加坡", "中国香港", "摩纳哥"]);
...
const out = {
  meta: {
    description: "Clean-pipeline v2 — PLI-anchored, zero Numbeo inputs",
    generated: new Date().toISOString(),
    formulaVersion: "clean-v2",
    anchor: `US_ANCHOR=$${US_ANCHOR} (BEA PCE core urban middle-class ex-housing)`,
    sources: {
      priceLevelIndex: "WB NY.GDP.PCAP.CD / NY.GDP.PCAP.PP.CD (CC BY 4.0)",
      rpp: "BEA Regional Price Parities 2022 (Public Domain)",
      pli: "Eurostat PLI 2022 (CC BY 4.0)",
      usRent: "Zillow ZORI (market median, all homes)",
...
const stats = { cost: 0, rentA: 0, rentB: 0, house: 0, missing: new Set() };
...
for (const city of sot.cities) {
  ...
  const pli = wbRec?.priceLevelIndex ?? imf?.priceLevelIndex ?? null;
  ...
  if (pli != null) {
    ...
    } else if (city.country in EU_ISO2_TO_CN || CN_TO_EU_ISO2[city.country]) {
      const iso2 = CN_TO_EU_ISO2[city.country];
      if (iso2 && inputs.eurostatPLI[iso2] != null) {
        cityPriceAdj = inputs.eurostatPLI[iso2] / 100; adjTag = "Eurostat-PLI";
      }
    }
    cost = Math.round(US_ANCHOR * pli * prem * cityPriceAdj);
    costSource = `${US_ANCHOR}×PLI(${pli.toFixed(2)})×prem(${prem})×${adjTag}(${cityPriceAdj.toFixed(2)})`;
    stats.cost++;
  } else {
  ...
  if (city.country === "美国" && zillow.cities[city.name]) {
    rent = zillow.cities[city.name].rentUSD;
    rentSource = `Zillow-ZORI-${zillow.month}`;
    stats.rentA++;
  } else if (city.country === "英国" && ons.cities[city.name]) {
    rent = Math.round(ons.cities[city.name].avgMonthlyRentGBP / fx.rates.GBP);
    rentSource = "ONS-2026";
    stats.rentA++;
  } else if (city.country === "加拿大" && statcan.cities[city.name]) {
    rent = Math.round(statcan.cities[city.name].rent1BR_CAD / fx.rates.CAD);
  ...
  out.cities.push({
    ...
    rent, rentSource,
    house,
    _inputs: { pli, gdpPPP, gni },
  });
}

console.log(`v2: cost ${stats.cost}/151, rent A=${stats.rentA}+B=${stats.rentB}=${stats.rentA + stats.rentB}/151, house ${stats.house}/151`);
if (stats.missing.size) console.log(`Missing: ${[...stats.missing].join(", ")}`);

writeFileSync(join(ROOT, "output/clean-values-v2.json"), JSON.stringify(out, null, 2) + "\n");
console.log(`\n✓ ${join(ROOT, "output/clean-values-v2.json")}`);

console.log("\nSample v2:");
const samples = ["纽约", "旧金山", "洛杉矶", "伦敦", "巴黎", "柏林", "东京", "新加坡", "香港", "多伦多", "悉尼", "曼谷", "孟买", "胡志明市", "开普敦", "墨西哥城"];
for (const nm of samples) {
  const c = out.cities.find(x => x.name === nm);
  if (!c) continue;
  console.log(`  ${nm.padEnd(10)} cost=$${String(c.cost).padStart(5)}  rent=$${String(c.rent).padStart(5)}  [${c.rentSource}]`);
}
