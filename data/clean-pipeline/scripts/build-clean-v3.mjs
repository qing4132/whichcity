#!/usr/bin/env node
/**
 * build-clean-v3.mjs — adds InsideAirbnb-calibrated rents to v2.
 *
 * Rent tier hierarchy (first match wins):
 *   A-gov  Zillow ZORI (US 19) / ONS (UK 2) / StatCan (CA 5)         — 26 cities
 *   A-airbnb  InsideAirbnb median × K (K fit from Tier A anchors)     — 22 cities
 *   B-formula  cost × rentShare(GDP-PPP)                              — remainder
 *
...
const out = {
  meta: {
    description: "Clean-pipeline v3 — PLI anchor + InsideAirbnb Tier A expansion",
    generated: new Date().toISOString(),
    formulaVersion: "clean-v3",
    ...
    sources: {
      ...
      airbnbRent: "InsideAirbnb median nightly × calibrated K=13.6 (K=10.2 tourist cities) (CC BY 4.0)",
...
const stats = { cost: 0, rentGov: 0, rentAirbnb: 0, rentFormula: 0, house: 0, missing: new Set() };
...
console.log(`v3: cost ${stats.cost}/151, rent gov=${stats.rentGov}+airbnb=${stats.rentAirbnb}+formula=${stats.rentFormula}=${stats.rentGov + stats.rentAirbnb + stats.rentFormula}/151`);
console.log(`house ${stats.house}/151`);
if (stats.missing.size) console.log(`Missing: ${[...stats.missing].join(", ")}`);

writeFileSync(join(ROOT, "output/clean-values-v3.json"), JSON.stringify(out, null, 2) + "\n");
console.log(`\n✓ output/clean-values-v3.json`);
