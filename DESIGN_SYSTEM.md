...
#### 城市详情页（CityDetailContent）— 百分位分级

| 档位 | 亮色卡片边框 | 暗色卡片边框 | 亮色数值 | 暗色数值 |
|------|-------------|-------------|---------|---------|
| good (前25%) | `border-emerald-400` | `border-emerald-500/60` | `text-emerald-600` | `text-emerald-400` |
| mid (中间50%) | `border-slate-200` | `border-slate-600` | 默认 headingCls | 默认 headingCls |
| bad (后25%) | `border-rose-400` | `border-rose-500/60` | `text-rose-500` | `text-rose-400` |
...
- 7 个 Tab 按钮等宽网格 (`grid-cols-4 sm:grid-cols-7`)
...
| Emoji | 使用位置 |
|-------|---------|
| 🇺🇸🇬🇧🇯🇵... | 城市列表、详情页、对比页 — 国旗 emoji（120条定义于 CITY_FLAG_EMOJIS） |
| 🏙️ | 默认城市图标（无国旗时） |
| ⭐ | 首页基准城市标识 |
| 🥇🥈🥉 | 排行榜 TOP3 |
| 🌙☀️ | 深色模式切换 |
...
**卡片标签**: 城市详情页的 12 张指标卡片标题区域**不使用 emoji**，纯文本标签。
...
| 5 | 排行榜阈值配色 vs 详情页百分位配色 | — | 两套系统并存是设计意图，但可能给用户造成困惑 |
